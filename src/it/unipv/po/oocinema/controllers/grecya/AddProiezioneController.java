...
public class AddProiezioneController implements Initializable {
	...
	/**
	 * Effettua una query per ottenere tutte le sale e le imposta nella relativa
	 * ChoiceBox
	 */
	private void setupSale() {
		...
		// Valore iniziale
		if (sale.size() > 0)
			choiceSala.setValue(sale.get(0));
	}

	/**
	 * Effettua una query per ottenere tutte le fascie orarie e le imposta nella
	 * relativa ChoiceBox
	 */
	private void setupFascieOrarie() {
		...
		// Valore iniziale
		if (fascieOrarie.size() > 0)
			choiceFasciaOraria.setValue(fascieOrarie.get(0));
	}

	/**
	 * Effettua una query per ottenere tutti i titoli e le imposta nella relativa
	 * ChoiceBox
	 */
	private void setupTitoli() {
		...
		// Valore iniziale
		if (films.size() > 0)
			choiceTitolo.setValue(films.get(0));
	}
	...
	public void addProiezione() {
		...
		Proiezione proiezione = new Proiezione(ID_film, dataAString, dataDaString, ID_sala, possibilitaOrario);
		...
		if (MySQLConnectionFactory.insertProiezione(proiezione)) {
			...
			alert = new Alert(AlertType.ERROR,
					"C'è stato un problema nell'aggiunta della proiezione, controlla i dati");
		}
